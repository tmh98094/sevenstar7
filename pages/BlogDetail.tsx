    ...
    if (!post) {
        return (
            <div className="min-h-screen bg-brand-black flex items-center justify-center flex-col p-6 text-center">
                <h1 className="text-3xl font-display font-black text-white mb-4">{t('blog.articleNotFound')}</h1>
                <p className="text-gray-400 mb-8">{t('blog.articleNotFoundDesc')}</p>
                <button onClick={() => navigate('/blog')} className="bg-brand-gold text-brand-black font-black px-8 py-3 rounded-full uppercase tracking-widest text-sm">{t('blog.back')}</button>
            </div>
        )
    }

    const translatedPost = (() => {
        const translated = t(`blog.${blogId}`) as any;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <div className="lg:col-span-2 prose prose-invert prose-sm md:prose-lg max-w-none prose-headings:font-display prose-headings:font-black prose-p:text-gray-300 prose-p:leading-relaxed prose-img:rounded-2xl prose-strong:text-white prose-a:text-brand-gold">
                            ...
                            <div className="mt-12 py-8 border-y border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
                                <span className="text-white font-black text-sm uppercase tracking-widest">{t('blog.shareThisGuide')}</span>
                        ...
                        <aside className="space-y-8">
                            ...
                            <div className="bg-brand-dark rounded-2xl p-8 border-2 border-brand-gold/20 text-center shadow-xl relative overflow-hidden group">
                                ...
                                <h3 className="text-xl md:text-2xl font-display font-black text-white mb-3">{t('ui.readyToWin')}</h3>
                                <p className="text-gray-400 text-sm mb-8 leading-relaxed">{t('partnerProgram.heroSubtitle')}</p>
                                <a
                                    href={EXTERNAL_LINK}
